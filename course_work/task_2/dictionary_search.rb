...
str = ARGV[0]
# Присвоение кажется бесполезным, но это нет так. Не знаю почему, но когда я пытался оставить здесь лишь вызов метода puts Dictionary.search(ARGV[0], 'lib/dictionary.txt')
# , то в консоли происходило приведённое ниже. Может это баг интерпретатора или я что-то не знаю. Надеюсь на ваш поясняющий комментарий.
# 
